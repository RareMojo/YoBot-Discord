...
async def update_yobot(yobot: 'YoBot') -> None:
    ...
    if yobot.config['update_bot'] == True:
        ...
        try: # Try to update the bot's name, presence, and avatar on the Discord servers.
            ...
            yobot.log.error('Failed to synchronize YoBot settings with Discord.')
...
async def welcome_to_yobot(yobot: 'YoBot') -> None:
    ...
    try:
        ...
        if not yobot.config['update_bot']: # If this is not the first run, print a welcome back message.
            ...
            yobot.log.info('Be sure to check out the documentation at the GitHub repository.')
            yobot.log.info('If you ever want to design a cog, check out the YoBot-Discord-Cog GitHub repository.')
            yobot.log.info('Type "help" for a list of terminal commands.')            
...
async def set_bot_presence(yobot: 'YoBot') -> None:
    ...
    try:
        ...
        if update_presence == True:
            ...
            update_config(yobot.config_file, {"presence": new_presence, "update_bot": True})
...
async def set_owner(yobot: 'YoBot') -> None:
    ...
    try:
        ...
        if change_owner_name == True:
            ...
            update_config(yobot.config_file, {"owner_name": new_owner_name, "owner_id": new_owner_id})

            yobot.log.info('Config change, owner_name: {} -> {}'.format(yobot.config['owner_name'], new_owner_name))
...
def github_repo_fetch(yobot: 'YoBot', owner: str, repo: str, repo_dir: str) -> list:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            yobot.log.debug(f"Failed to fetch files from {owner}/{repo}/{repo_dir}. Response status code: {response.status_code}")
            return []
    except Exception as e:
        yobot.log.error(f"An error occurred while fetching files from {owner}/{repo}/{repo_dir}: {e}")
        return []
...
def download_cogs(yobot: 'YoBot', cogs_dir: str, sigs_dir: str) -> None:
    ...
    try:
        ...
        get_cogs = get_boolean_input(yobot, 'Do you want to download cogs? (y/n) ')
        successful = False

        if get_cogs == True:
            get_all_cogs = get_boolean_input(yobot, 'Do you want to download all cogs? (y/n) ')
            ...
            if get_all_cogs == True:
                ...
                if files:
                    ...
                    successful = True
                    yobot.log.debug('Downloading selected cogs and their signatures from the repository...')
                        ...
                        for file in os.listdir(cogs_dir):
                            if file.endswith('cog.py') and file not in yobot.cogs_removal_blacklist:
                                try:
                                    ...
                                    yobot.log.debug(f"Removed {file} from {cogs_dir}")
                                except Exception as e:
                                    yobot.log.error(f"Error occurred while removing {file}: {e}")
                else:
                    yobot.log.info('Cogs not uninstalled.')               
            else:
                yobot.log.info('Fetching the list of cogs from the cogs directory...')
                files = [file for file in os.listdir(cogs_dir) if file.endswith('cog.py') and file not in yobot.cogs_removal_blacklist]
                ...
                if confirm_removal == True:
                    ...
                    for cog_index in selected_cogs:
                        ...
                        try:
                            ...
                            yobot.log.debug(f"Removed {cog_name} from {cogs_dir}")
                            yobot.log.info(f'{cog_name} uninstalled.')
                        except Exception as e:
                            yobot.log.error(f"Error occurred while removing {cog_name}: {e}")
...
def list_cogs(yobot: 'YoBot', cogs_dir: str) -> list:
    ...
    """
    try:
        yobot.log.debug(f"Fetching list of installed cogs from directory '{cogs_dir}'...")
        ...
        if not files:
            ...
            for i, file in enumerate(files, start=1):
                yobot.log.info(f'{i}. {file}')
            yobot.log.debug(f"List of installed cogs fetched successfully from directory '{cogs_dir}'.")
            return files
    except FileNotFoundError:
        yobot.log.error(f"The directory '{cogs_dir}' does not exist.")
        return []
    except Exception as e:
        yobot.log.error(f"An error occurred while fetching the list of installed cogs: {e}")
        return []
    ...
    try:
        ...
        with open(public_key_file, 'rb') as file:
            ...
            if not isinstance(public_key, RSAPublicKey):
                yobot.log.error(f"The public key in the file '{public_key_file}' is not an RSA key.")
                return None
                
        yobot.log.debug(f"Public key loaded successfully from file '{public_key_file}'.")
        return public_key
    except FileNotFoundError:
        yobot.log.error(f"The public key file '{public_key_file}' does not exist.")
        return None

    except Exception as e:
        yobot.log.error(f"An error occurred while loading the public key from file '{public_key_file}': {e}")
        return None
...
def verify_cog_signature(yobot: 'YoBot', cog_file: str, signature_file: str, public_key) -> bool:
    ...
    try:
        ...
        yobot.log.debug(f"YoBot attempted to read the cog file at '{cog_file}' but could not find it.")
        return False
    ...
    try:
        ...
        yobot.log.error(f"The signature file '{signature_file}' does not exist.")
        yobot.log.debug(f"YoBot attempted to read the signature file at '{signature_file}' but could not find it.")
        return False
    ...
    try:
        ...
        yobot.log.debug(f"The signature is valid for the cog file '{cog_file}'.")
        return True
    except ValueError as e:
        yobot.log.error(f"The signature is invalid for the cog file '{cog_file}'.")
    ...
    try:
        ...
        for cog in cogs:
            ...
            if not os.path.exists(signature_path):
                ...
                yobot.log.debug(f"Expected signature at {signature_path} for cog at {cog_path}.")
                all_cogs_valid = False
                continue

            is_valid = verify_cog_signature(yobot, cog_path, signature_path, public_key)

            if is_valid:
                yobot.log.info(f"{cog} signature is valid.")
            else:
                yobot.log.error(f"{cog} signature is NOT valid.")
                yobot.log.debug(f"Signature at {signature_path} is not valid for cog at {cog_path}.")
                all_cogs_valid = False

        if not all_cogs_valid:
            yobot.log.debug(f"YoBot has encountered a fatal error during cog verification.")
        else:
...
def toggle_debug_mode(yobot: 'YoBot') -> None:
    ...
    try:
        ...
        yobot.log.warning(f"Error loading config file {yobot.config_file}: {e}")
    except Exception as e:
...
def add_blacklist(yobot: 'YoBot') -> None:
    ...
    if not edit_confirm:
        ...
        if blacklist_choice == '1':
            ...
            cog_name = input('Enter the name of the cog you would like to add: ')

            blacklist = config.get('blacklist', {}).get('cog_verify', [])

            if cog_name in blacklist:
                yobot.log.warning(f"'{cog_name}' is already in the cog verification blacklist.")
                return
            ...
            yobot.log.info(f"'{cog_name}' has been added to the cog verification blacklist.")
    ...
    try:
        ...
        if not edit_confirm:
            ...
            if blacklist_choice == '1':
                ...
                yobot.log.info('Here are the current cogs in the cog removal blacklist:')
                for i, cog in enumerate(blacklist_cog_removal):
                    yobot.log.info(f'{i+1}. {cog}')
                cog_number = int(input('Enter the number of the cog you would like to remove: '))
                ...
                yobot.log.info('Here are the current cogs in the cog verify blacklist:')
                ...
                yobot.log.warning(f"'{blacklist_choice}' is not a valid choice.")
                return yobot.log.warning('Blacklist edit aborted.')
            ...
            yobot.log.info(f"'{cog_name}' has been removed from the chosen blacklist.")
