...
class YoBotBuilder(YoBot):
    ...
    def __init__(self, config_file: str):
        ...
        self.log = YoBotLogger(name='YoBot', log_file=self.log_file,
                               level=self.config['log_level'], maxBytes=1000000, backupCount=1)
            ...
            if not os.path.isdir(self.cogs_dir):
                ...
                if config['update_bot']:
                    ...
                    download_cogs(self, self.cogs_dir,
                                  self.cogs_sigs_dir, self.repo_info)
    ...
    def yobot_build(self):
        ...
        try:
            self.setup_cogs()
            # Skip signature check if dev_mode is enabled.
            if self.config['dev_mode'] == True:
                self.log.info(
                    'Dev mode is enabled. Skipping cog verification.')
                self.log.warning(
                    'WARNING: This can allow malicious code to run on your system!')
            else:
                self.log.debug('Dev mode is not enabled. Verifying cogs...')

                # Verify all cogs against their signatures to ensure they have not been tampered with.
                if verify_all_cogs(self, self.cogs_dir, self.cogs_key_file, self.config['blacklist']['cog_verify']) == False:
                    self.log.warning(
                        'YoBot has detected that one or more cogs have been tampered with.')
                    self.log.warning(
                        'This can be caused by a malicious user or a bug in the YoBot code.')
                    self.log.warning(
                        'If you are a user, please contact the Cog author or the developer of your YoBot.')
                    self.log.warning(
                        'If you are a developer, you can disable this check by enabling dev_mode in the config file.')
                    self.log.critical(
                        'WARNING: Disabling this check can allow malicious code to run on your system!')
                    self.log.critical(
                        'WARNING: Only disable this check if you are a developer and know what you are doing!')
